...
Despite the miniboxing annotations, all versions of the |tupleMap| method use the erased generic versions of the tuple accessors and the function application, leading to slow paths irreversibly creeping into miniboxed code. For many applications, this is a no-go, so our task was to eliminate these cases. In the following subsection we present three possible approaches and show where each works best.
